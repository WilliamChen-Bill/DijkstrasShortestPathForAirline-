...
public class CS400Graph<T> implements GraphADT<T>{ 
    ...
    protected Path dijkstrasShortestPath(T start, T end) { 
        // if can't found the start vertex or end vertex fail
        if(!containsVertex(start) || !containsVertex(end)){
            throw new NoSuchElementException("can't found path");
        }
        Path startPath = new Path(vertices.get(start));
        Path extendPath;
        Path shortestPath = startPath;
        Vertex currentVertex;
        //hold discovered path
        ...
        while(!queue.isEmpty()){
            ...
            for(Edge edge : edges){
                for(T data : shortestPath.dataSequence){
                    if(edge.target.data == data){
                        break;
                    }
                    extendPath = new Path(shortestPath, edge);
                    queue.add(extendPath);
                }
